...
class PurchaseRequestLine(models.Model):
    ...
    assigned_accounts_user_id = fields.Many2one(
        comodel_name="res.users",
        related="request_id.assigned_accounts_user_id",
        string="Assigned to",
        store=True,
    )
    assigned_unit_manager_user_id = fields.Many2one(
        comodel_name="res.users",
        related="request_id.assigned_unit_manager_user_id",
        string="Assigned to",
        store=True,
    )
    assigned_manager_user_id = fields.Many2one(
        comodel_name="res.users",
        related="request_id.assigned_manager_user_id",
        string="Assigned to",
        store=True,
    )
    ...
    def _compute_is_editable(self):
        for rec in self:
            if rec.request_id.state in ("to_approve", "accounts_approved", "unit_manager_approved", "manager_approved", "rejected", "done"):
