odoo.define('sales_circulation.circulation_agent_dashboard', function (require){
"use strict";
...
var SalesCirculationDashBoard = AbstractAction.extend({
   template: 'circulation_agent_dashboards',
   init: function(parent, context) {
        this._super(parent, context);
        this.dashboards_templates = ['circulation_agent_dashboard_data'];
        this.today_sale = [];
    },
    willStart: function() {
    var self = this;
    ...
    start: function() {
        ...
        _.each(this.dashboards_templates, function(template) {
                self.$('.o_agent_dashboard').append(QWeb.render(template, {widget: self}));
            });
...
//                       borderWidth: 1,
    ...
    fetch_data: function() {
...

        {
            ...
            self.agent_week_indent = result['agent_week_indent'],
            self.week_date_range = result['week_date_range'],
...
core.action_registry.add('circulation_agent_dashboard_tags', SalesCirculationDashBoard);
