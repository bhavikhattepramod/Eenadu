...
class DemandRequest(models.Model):
    ...
    types_of_copies = fields.Selection(
        selection=[
            ('agent', "Agent Copies"),
            ('free', "Free Copies"),
            ('voucher', "Voucher Copies"),
            ('promotional', "Promotional Copies"),
            ('correspondents', "Correspondents Copies"),
            ('office', "Office Copies"),
            ('postal', "Postal Copies"),
        ],
        string="Type of Copies",
        index=True,
        tracking=True, )
    ...
    def _onchange_total_copies_specific_date(self):
        ...
        for record in self:
            if record.selection_additional_type == 'increase_additional':
                record.total_copies = record.Agent_copies + record.Agent_id.f_q_zone + record.Agent_id.p_q_zone + record.Agent_id.v_q_zone + record.Agent_id.pr_q_zone + record.Agent_id.c_c_zone + record.Agent_id.o_q_zone + record.no_of_additional_copies
            elif record.selection_additional_type == 'decrease_additional':
                record.total_copies = record.Agent_copies + record.Agent_id.f_q_zone + record.Agent_id.p_q_zone + record.Agent_id.v_q_zone + record.Agent_id.pr_q_zone + record.Agent_id.c_c_zone + record.Agent_id.o_q_zone - record.decrease_additional_copies
            else:
                record.total_copies = record.Agent_copies
