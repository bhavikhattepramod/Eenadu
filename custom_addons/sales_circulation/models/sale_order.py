                ...
                for demand in demand_request:
                    if demand and demand.state == 'approved':
                        if demand.no_of_additional_copies:
                            if demand.types_of_copies == 'agent':
                                total = lines.agent_copies + demand.no_of_additional_copies
                                total_copies = lines.product_uom_qty + demand.no_of_additional_copies
                                total_demand_changes += demand.no_of_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                                    'agent_copies': total,
                                    'demand_added': total_demand_changes,
                                    'demand_state': 'increase',
                                    'magazine': total,
                                })
                            elif demand.types_of_copies == 'free':
                                total = lines.free_copies + demand.no_of_additional_copies
                                total_copies = lines.product_uom_qty + demand.no_of_additional_copies
                                total_demand_changes += demand.no_of_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                            ...
                            elif demand.types_of_copies == 'voucher':
                                total = lines.voucher_copies + demand.no_of_additional_copies
                                total_copies = lines.product_uom_qty + demand.no_of_additional_copies
                                total_demand_changes += demand.no_of_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                                    'voucher_copies': total,
                                    'demand_added': total_demand_changes,
                                    'demand_state': 'increase',
                                    'magazine': total,
                                })
                            elif demand.types_of_copies == 'promotional':
                                total = lines.promotional_copies + demand.no_of_additional_copies
                                total_copies = lines.product_uom_qty + demand.no_of_additional_copies
                                total_demand_changes += demand.no_of_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                                    'promotional_copies': total,
                                    'demand_added': total_demand_changes,
                                    'demand_state': 'increase',
                                    'magazine': total,
                                })
                            elif demand.types_of_copies == 'correspondents':
                                total = lines.correspondents_copies + demand.no_of_additional_copies
                                total_copies = lines.product_uom_qty + demand.no_of_additional_copies
                                total_demand_changes += demand.no_of_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                                    'correspondents_copies': total,
                                    'demand_added': total_demand_changes,
                                    'demand_state': 'increase',
                                    'magazine': total,
                                })
                            elif demand.types_of_copies == 'office':
                                total = lines.office_copies + demand.no_of_additional_copies
                                total_copies = lines.product_uom_qty + demand.no_of_additional_copies
                                total_demand_changes += demand.no_of_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                            ...
                            elif demand.types_of_copies == 'postal':
                                total = lines.postal_copies + demand.no_of_additional_copies
                                total_copies = lines.product_uom_qty + demand.no_of_additional_copies
                                total_demand_changes += demand.no_of_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                            ...
                            if demand.types_of_copies == 'agent':
                                total = lines.agent_copies - demand.decrease_additional_copies
                                total_copies = lines.product_uom_qty - demand.decrease_additional_copies
                                ...
                                total = lines.free_copies - demand.decrease_additional_copies
                                total_copies = lines.product_uom_qty - demand.decrease_additional_copies
                                total_demand_changes += demand.decrease_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                                ...
                                total = lines.voucher_copies - demand.decrease_additional_copies
                                total_copies = lines.product_uom_qty - demand.decrease_additional_copies
                                total_demand_changes += demand.decrease_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                                ...
                                total = lines.promotional_copies - demand.decrease_additional_copies
                                total_copies = lines.product_uom_qty - demand.decrease_additional_copies
                                total_demand_changes += demand.decrease_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                                ...
                                total = lines.correspondents_copies - demand.decrease_additional_copies
                                total_copies = lines.product_uom_qty - demand.decrease_additional_copies
                                total_demand_changes += demand.decrease_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                                ...
                                total = lines.office_copies - demand.decrease_additional_copies
                                total_copies = lines.product_uom_qty - demand.decrease_additional_copies
                                total_demand_changes += demand.decrease_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
                                ...
                                total = lines.postal_copies - demand.decrease_additional_copies
                                total_copies = lines.product_uom_qty - demand.decrease_additional_copies
                                total_demand_changes += demand.decrease_additional_copies
                                lines.update({
                                    'product_uom_qty': total_copies,
