...
class HrPayslip(models.Model):
    ...
    def create(self, vals):
        exist_payslip = self.env['hr.payslip'].search([('employee_id', '=', vals.get('employee_id'))])
        exit_list = []
        for exist in exist_payslip:
            if str(exist.date_from) == str(vals.get('date_from')):
                if str(exist.date_to) == str(vals.get('date_to')):
                    exit_list.append(exist)
        if exit_list:
            raise ValidationError('Payslip is already exist.Please update in the existing payslip')
        else:
            pass
    ...
    # def write(self, vals):
    #     for rec in self:
    #         exist_payslip = self.env['hr.payslip'].search([('employee_id', '=', rec.employee_id.id)])
    #         exit_list = []
    #         for exist in exist_payslip:
    #             if str(exist.date_from) == str(rec.date_from):
    #                 if str(exist.date_to) == str(rec.date_to):
    #                     exit_list.append(exist)
    #         if exit_list:
    #             raise ValidationError('Payslip is already exist.Please update in the existing payslip')
    #         else:
    ...
    def _cron_generate_payslips(self):
        ...
        exist_payslip = self.env['hr.payslip.run'].search([('date_start', '=', date_from), ('date_end', '=', date_to)])
        if not exist_payslip:
    ...
    def action_payslip_done(self):
        ...
        template_id = self.env.ref(
            'om_hr_payroll.mail_template_payslip').id
