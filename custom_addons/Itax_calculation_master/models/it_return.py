...
class ItReturns(models.Model):
    ...
    def list_all_months(self):
        data_list = []
        # Retrieve the financial start and end dates from the res.company model
        company = self.env['res.company'].browse(self.env.company.id)
        financial_start_date = company.fiscal_year_start_date
        financial_end_date = company.fiscal_year_last_date
        current_date = financial_start_date

        while current_date <= financial_end_date:
    ...
    employee_id = fields.Many2one("hr.employee", string="Id No")
    employee_code = fields.Char("Employee Name", related='employee_id.identification_id')
    ...
    gen_months = fields.Float('Check Total')
    actual_total_pay = fields.Float('Payslip Total')
    rem_months = fields.Integer('')
    ...
    def _onchange_employee_id(self):
        for rec in self:
            try:
                ...
                employee_rec = False
            if employee_rec:
                if rec.employee_id.tax_regim_type.name == 'Old Tax Regime':
    ...
    # For calculating the gross total
    ...
    def compute_tax_sheet(self):
        for rec in self:
            start_financial = self.env.company.fiscal_year_start_date
            end_financial = self.env.company.fiscal_year_last_date
            diff_dates = ((end_financial.year - start_financial.year) * 12 + (
                        end_financial.month - start_financial.month) + 1)
            # print(diff_dates,'vvvvvvvvvvvvvvvvvvvvvvvvvvv')
            pay_slip_id = self.env['hr.payslip'].search([('employee_id', '=', rec.employee_id.id)], order='id desc',
                                                        limit=1)
            count_payslip = self.env['hr.payslip'].search([('employee_id', '=', rec.employee_id.id)])
            rec.rem_months = ((end_financial.year - pay_slip_id.date_from.year) * 12 + (
                        end_financial.month - pay_slip_id.date_from.month))
            # print(rec.rem_months, 'rrrrrrrrrrrrrrrrrrrr')
            payslip_months = [payslip.date_from.month for payslip in count_payslip]
            # print(payslip_months,'payslip_months')
            # total_val = master_total = 0
            basic_it = 0
            cur_month = self.tds_computation_month
            if pay_slip_id:
                ...
                # print(count_payslip.date_from.month,'count_payslip.date_from.monthhhhhhhhhhhh')
                if datetime.strptime(cur_month, "%b-%Y").month in payslip_months:
                    if rec.tax_resign_type.name == 'Old Tax Regime':
                        if rec.gross_line_ids:
                            for lines in rec.gross_line_ids:
                                if lines.gross_annual_income.salary_rule:
                                    # for vals in pay_slip_id.line_ids:
                                    #     # print(vals.name, 'comp')
                                    #     if vals.tax_bool:
                                    #         rec.actual_total_pay += vals.total
                                    #         print(rec.actual_total_pay,'ghhhhhghghghghhghgh')
                                    #         master_total += vals.salary_master
                                    #         rec.gen_months = rec.actual_total_pay
                                    #         # print(master_total,'mmmmmmmmmmmmmmmmmmmmmmm')
                                    #         total_val = rec.gen_months
                                    #         print(total_val,'ttttttttttttttttttttt')
                                    # lines.amount = total_val
                                    # print(lines.amount,'adgcvmjhvjhmvcmje ')
                                    # gross_total = lines.amount
                                    # print(gross_total,'nnnnnnnnnnnnnnnnnnnnnnnnnnn')
                                    # Creating a empty dictionary to keep track of processed rules
                                    processed_rules = {}
                                    for rule_line in lines.gross_annual_income.salary_rule:
                                        for rules in count_payslip.line_ids:

                                            if rule_line.code == rules.code and rules.amount != 0.00 and rules.salary_rule_id.is_taxable == True:
                                                # Construct a unique identifier for the rule, you can modify this based on your use case
                                                rule_identifier = (rules.code, rules.salary_rule_id.is_taxable)

                                                # Check if the rule has been processed before
                                                if rule_identifier in processed_rules:
                                                    # Update the existing record
                                                    existing_record = processed_rules[rule_identifier]
                                                    existing_record['amount'] += rules.amount
                                                    existing_record['amount_total'] += rules.total
                                                    existing_record[
                                                        'salary_total'] = rules.salary_master * rec.rem_months
                                                else:
                                                    # Create a new record and add it to the processed rules dictionary
                                                    new_record = {
                                                        'name': rules.name,
                                                        'amount': rules.amount,
                                                        'salary_total': rules.salary_master * rec.rem_months,
                                                    ...
                                                    processed_rules[rule_identifier] = new_record
                                    # Now you can use the processed_records dictionary to update or create records as needed
                                    for rule_identifier, record_data in processed_rules.items():
                                        if record_data['amount'] != 0.00:
                                            record_data['amount'] += record_data['salary_total']
                                            record_data['amount_total'] += record_data['salary_total']
                                            rec.update({
                                                'computed_line_ids': [(0, 0, record_data)],
                                            })
                                            gross_total += record_data['amount']
                                            lines.amount = gross_total
                                            # print(record_data,'rdrdrdrdrdrdrdrdrdrdrdrdrdrd')
                                            # print(processed_rules,'prprprpprprrrprrp')
                                    ...
                                    gross_total += lines.amount
                                    print(gross_total, 'llllllllllllllllllllllllllllllllll')
                            rec.update({
                            ...
                            print(gross_total, 'hjhfgdgfhjkgffhgh')
                        ...
                        if rec.section_10_line_ids:
                            ...
                            for lines in rec.section_10_line_ids:
                                if 'House Rent Allowance' in lines.section_10.name:
                                    if rec.employee_id.rent_paid:
                                        # other than HRA anything need to added it should be added here
                                        actual_hra_received = processed_rules[('HRA', True)]['amount']
                                        print(actual_hra_received, 'acacaacacacacaccaa')
                                        basic_da = processed_rules[('BASIC', True)]['amount'] + \
                                                   processed_rules[('DA', True)]['amount']
                                        amount_from_basic = (basic_da * (
                                                rec.employee_id.city_type_conf.percent / 100))
                                        rent_paid = (rec.employee_id.rent_paid * 12) - (basic_da * (10 / 100))
                                        ...
                                        #                                   })],
                                        # })
                                        print(lines.section_10.name, 'nannaansankjsnhjasbabab')
                                        lines.update({
                            ...
                            rec.update({
                                'computed_line_ids': [
                                    (0, 0, {'name': 'Total amount of exemption claimed under section 10',
                                            'amount': sec10_total,
                                            'color': 'bold',
                        ...
                        if oth_in_total:
                            rec.update({
                                'computed_line_ids': [
                                    (0, 0, {'name': 'Income under the head Other Sources offered for TDS',
                                            'amount': oth_in_total,
                                            'color': 'bold',
                                ...
                                if lines.max_limit and lines.amount:
                                    rec.update({
                                        'computed_line_ids': [(0, 0, {'name': lines.section_24.name,
                                                                      ...
                                                                      'amount_total': max(lines.max_limit,
                                                                                          lines.amount),
                                                                      'color': 'blue'
                                                                      })],
                                    })
                                ...
                                if lines.amount and lines.max_limit:
                                    rec.update({
                                        'computed_line_ids': [(0, 0, {'name': lines.chapter6.name,
                                                                      ...
                                                                      'amount_total': min(lines.max_limit,
                                                                                          lines.amount),
                                                                      'color': 'blue'
                                                                      })],
                                    })
                        ...
                        if sec80c_total and chp6_total:
                            rec.update({
                                'computed_line_ids': [
                                    (0, 0, {'name': 'Aggregate of deductible amount under Chapter VI-A',
                        ...
                        current_date = datetime.now()
                        # print(current_date, 'current_dateeeeeeeeeeeeeeeeeeeeee')
                        month = self.tds_computation_month
                        # print(month, 'monthhhhhhhhhhhhhhhh')
                        start = self.env.company.fiscal_year_start_date
                        end = self.env.company.fiscal_year_last_date
                        date_list = []
                        while start < end:
                            date_list.append(start)
                            # print(date_list, 'date_listtttttttttttttttt')
                            start += relativedelta(months=1)
                        tds_id = self.env['employee.tds'].search([('employee_id', '=', rec.employee_id.id),
                                                                  ('month', '=', month)])
                        pop_list = []
                        for dt in range(len(date_list)):
                            if date_list[dt] < current_date.date():
                                pop_list.append(dt)
                                # print(pop_list, 'pop_listtttttttttt')
                        range_val = len(pop_list) - 1
                        for pd in range(range_val):
                            if len(date_list) == pd - 1:
                                date_list.pop(pd)
                                # print(date_list, 'date_listddddddddddddddddddd')
                        fut_months = datetime.strptime(rec.tds_computation_month, '%b-%Y')
                        future_months = rec.rem_months + 1
                        # print(future_months, 'fffffffffffffffffffffff')
                        recovered_amounts = self.report_recovered_so_far()
                        # print(recovered_amounts, 'recovered_amounts')
                        # print(month,'monthhsgagsahgsadvsvvhsvhsavchgasvcghasvchgsvhdvhdsfhgd')
                        pay_slip = self.env['hr.payslip'].search([('employee_id', '=', rec.employee_id.id),
                                                                  ...
                                                                  ('date_to', '>=',
                                                                   datetime.strptime("2-" + month, "%d-%b-%Y"))],
                                                                 limit=1)
                        # print(pay_slip,'pay_slipppppppppppppppppp')
                        if tds_id:
                            tds_id.update({
                                'date': datetime.strptime(rec.tds_computation_month, "%b-%Y").replace(day=1),
                            ...
                            # print(tds_id['date'], tds_id['income_tax'], tds_id['surcharge'], tds_id['cess'],
                            #       tds_id['total_amount'], tds_id['payslip_id'], 'tds_iddddddddddddddddddd')
                        else:
                            tds_id = self.env['employee.tds'].create({
                                'employee_id': rec.employee_id.id,
                                'month': month,
                            ...
                            # print(tds_id['employee_id'], tds_id['month'], tds_id['date'], tds_id['income_tax'],
                            #       tds_id['surcharge'], tds_id['cess'], tds_id['total_amount'], tds_id['payslip_id'],
                            #       'tds_idtttttttttttttttttttt')
                        ...
                        if rec.gross_line_ids:
                            for lines in rec.gross_line_ids:
                                if lines.gross_annual_income.salary_rule:
                                    ...
                                    # print(gross_total, 'Gross total4')
                                            ...
                                            if rule_line.code == rules.code:
                                                if rules.amount != 0.00:
                                                    if rules.salary_rule_id.rule_type == 'fixed':
                                                        ...
                                                        # amount is updating in the gross annual income
                                                        lines.update({
                                                            'amount': gross_total,
                                                        })
                                                        print(gross_total, 'Gross total3')
                                                    else:
                                                        rec.update({
                                                            'computed_line_ids': [(0, 0, {'name': rules.name,
                                                                                          'amount': rules.amount,
                                                        ...
                                                        print(gross_total, 'Gross total2')
                                    ...
                                    gross_total += lines.amount
                                    print(gross_total, 'Gross total1')
                            rec.update({
                                'computed_line_ids': [(0, 0, {'name': 'Total Gross Salary',
                                                              'amount': gross_total,
                                                              'color': 'bold',
                            ...
                            # print(gross_total, 'rtytrertrtrtrt')
                    ...
                    for line_data in rec.computed_line_ids:
                        ...
                        # print(com_data_list, 'com_data_list')
    ...
    def report_recovered_so_far(self):
        ...
        for tds in emp_tds_ids:
            ...
            print(i_tax, 'i_tax', surcharge, 'surcharge', cess, 'cess', total, 'totallllllllllllllllllll')
        return {
            'i_tax': i_tax,
            'surcharge': surcharge,
            'cess': cess,
    ...
    def report_recovery_for_the_month(self):
        ...
        if emp_tds_id:
            ...
            print(i_tax, 'i_tax', surcharge, 'surcharge', cess, 'cess', total, 'total')
        else:
            return {
                'i_tax': 0.00,
                'surcharge': 0.00,
        ...
        print(i_tax, 'i_taxxxxx', surcharge, 'surchargeeee', cess, 'cessssss', total, 'totalllllll')

    def report_month(self):
        emp_tds_id = self.env['employee.tds'].search(
            [('employee_id', '=', self.employee_id.id), ('date', '=', datetime.today().replace(day=1))], limit=1)
        print(emp_tds_id, 'emp_tds_iddddddddddd')
        if emp_tds_id:
            print(emp_tds_id.month, 'emp_tds_id.monthhhhhhhh')
    ...
    def report_balance_payable(self):
        ...
        print(tds_payable, 'tttttttttttttttttttttt')
        line_id = self.env['computed.lines'].search([('it_returns_computed', '=', self.id),
                                                     ('name', '=', 'Total Tax Payable')])
        if line_id and tds_payable:
            print(line_id.amount_total, 'line_id.amount_total', tds_payable.get('total'), "tds_payable.get('total')")
    ...
    def report_get_months(self):
        ...
        while start < end:
            date_list.append(start)
            print(start, 'startttttttttttttttttttttttt')
            start += relativedelta(months=1)
            print(start, 'startjjjjjjjjjjjjjjjjjjjjjjjj')
    ...
    def get_taxed_months(self):
        for emp_id in self:
            taxed_months = self.env['employee.tds'].search([('employee_id', '=', emp_id.employee_id.id)])
            data = []
            for rec in taxed_months:
                month_data = {
                    'month': rec.month,
    ...
    def calculate_total_tax(self):
        data = self.get_taxed_months()
        total_income_tax = sum(item['income_tax'] for item in data)
        total_surcharge = sum(item['surcharge'] for item in data)
        total_cess = sum(item['cess'] for item in data)
        return {
...
class ComputedLines(models.Model):
    ...
    salary_total = fields.Float("Final Total")
