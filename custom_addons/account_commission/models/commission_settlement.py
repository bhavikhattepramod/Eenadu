...
class CommissionSettlement(models.Model):
    ...
    # for classified commission if elif
    def _prepare_invoice(self, vendor, journal, product, date=False):
        ...
        for settlement in self:
            ...
            if settlement.commission_for == 'reta':
                print(settlement.commission_for,'lalalala')
                vals["invoice_line_ids"].append(
                    (
                        ...
                        0,
                        {
                            ...
                            "price_unit": abs(settlement.total_commission),
                            "name": (
                                "\n"
                                + _(
            ...
            #     for classified
            elif settlement.commission_for == 'classified':
                print(settlement.commission_for)
                vals["invoice_line_ids"].append(
                    (
                        0,
                        0,
                        {
                            "product_id": product.id,
                            "quantity": -1 if settlement.total < 0 else 1,
                            "price_unit": abs(settlement.total_commission_classified),
                            "name": (
                                "\n"
                                + _(
                                    "Period: from %(date_from)s to %(date_to)s",
    ...
    def make_invoices(self, vendor, journal, product, date=False, grouped=False):
        invoice_vals_list = []
        ...
        for settlement in grouped_settlements:
            invoice_vals = settlement._prepare_invoice(vendor, journal, product, date)
