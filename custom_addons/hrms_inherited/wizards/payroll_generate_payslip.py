    ...
    def end_date_day(self):
        if self.date_from:
            self.date_to = fields.Date.to_string(
                self.date_from + relativedelta.relativedelta(months=+1, day=1, days=-1))

    def generate_payslip(self):
        payslip = self.env['hr.payslip']
        employee = self.env['hr.employee'].search([('stop_payment_payroll', '=', False)])
            ...
            if not exist_list:
                ...
                if not rec:
                    if emp.contract_id.state == 'open':
                        ...
                        print(new_rec.compute_sheet,'generatecompute')
